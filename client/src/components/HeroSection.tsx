    ...
    <section className="relative h-screen overflow-hidden" id="hero">
      ...
      <div className="relative z-10 h-full flex items-center">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 w-full">
          <div className="max-w-2xl">
            ...
            <p className={`font-[Montserrat] text-[15px] leading-relaxed text-[#8A8279] mt-8 max-w-lg transition-all duration-1000 delay-700 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
              {t("hero.desc")}
            </p>
            ...
            <div className={`flex flex-wrap gap-4 mt-10 transition-all duration-1000 delay-900 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
              <a href="#services" className="inline-flex items-center font-[Montserrat] text-[12px] uppercase tracking-[0.15em] text-[#0A0A0A] bg-[#C9A84C] px-8 py-3.5 rounded-full hover:bg-[#E8D5B7] transition-colors duration-300">
                {t("hero.cta1")}
              </a>
              <a href="#about" className="inline-flex items-center font-[Montserrat] text-[12px] uppercase tracking-[0.15em] text-[#C9A84C] border border-[#C9A84C]/40 px-8 py-3.5 rounded-full hover:border-[#C9A84C] hover:bg-[#C9A84C]/10 transition-all duration-300">
                {t("hero.cta2")}
              </a>
            </div>
          </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
        <a href="#about" className="flex flex-col items-center gap-2 text-[#C9A84C]/60 hover:text-[#C9A84C] transition-colors duration-300 group" aria-label="Scroll down">
          <span className="font-[Montserrat] text-[10px] uppercase tracking-[0.3em]">{t("hero.scroll")}</span>
